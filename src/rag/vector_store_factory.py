        ...
        **kwargs
        ...
        if vector_store_provider.lower() == 'qdrant':
            try:
                ...
                logger.info("Qdrant support is not yet implemented. Falling back to ChromaDB.")
                st.warning(f"{vector_store_provider.lower()} not implemented")
            except ImportError:
                logger.warning("Qdrant package not found. Falling back to ChromaDB.")
                
        elif vector_store_provider.lower() == 'pinecone':
            try:
                # Future implementation for Pinecone
                logger.info("Pinecone support is not yet implemented. Falling back to ChromaDB.")
                st.warning(f"{vector_store_provider.lower()} not implemented")
            except ImportError:
                logger.warning("Pinecone package not found. Falling back to ChromaDB.")
        
                
        elif vector_store_provider.lower() == 'weaviate':
            try:
                # Future implementation for Weaviate
                logger.info("Weaviate support is not yet implemented. Falling back to ChromaDB.")
                st.warning(f"{vector_store_provider.lower()} not implemented")

            except ImportError:
                logger.warning("Weaviate package not found. Falling back to ChromaDB.")
