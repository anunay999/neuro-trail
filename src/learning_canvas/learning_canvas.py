...
class LearningCanvas:
    ...
    def add_epub(self, epub_file, user_id="user_123") -> bool:
        ...
        # If vector store service not initialized, try to initialize with default values
        if self.vector_store_service is None and self.embedding_model is not None:
            logger.info("Vector store service not initialized, initializing now")
            self.vector_store_service = VectorStoreService(
                chapter_mode=True
            )
    ...
    def search_query(self, query, top_k=3):
        ...
        # Validate vector store service is initialized
        if self.vector_store_service is None:
            logger.error("Vector store service not initialized. Please set models first.")
            st.toast("Vector store service not initialized. Please set models first.", icon="⚠️")
            return results
