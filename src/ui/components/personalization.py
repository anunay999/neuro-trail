...
def personalization_ui():
    """
    Render the personalization settings UI as a tab component.
    This includes response style settings and digest configuration.
    """
    ...
    create_template = st.checkbox("Create new template", 
    ...
    if st.session_state.get("show_template_editor", False):
        with st.form("template_form"):
            ...
            template_desc = st.text_area("Description", value="Custom response style", height=70)
            ...
            template_vars = st.text_input(
                ...
                value="context,question,style,length,expertise"
            )
            
            submit_template = st.form_submit_button("Save Template", use_container_width=True)
